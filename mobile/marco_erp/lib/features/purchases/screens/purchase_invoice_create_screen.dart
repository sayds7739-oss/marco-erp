...
class _PurchaseInvoiceLineItem {
  int? productId;
  String? productName;
  double quantity;
  double unitPrice;
...
class PurchaseInvoiceCreateScreen extends StatefulWidget {
  ...
  @override
  State<PurchaseInvoiceCreateScreen> createState() => _PurchaseInvoiceCreateScreenState();
}

class _PurchaseInvoiceCreateScreenState extends State<PurchaseInvoiceCreateScreen> {
  ...
  final List<_PurchaseInvoiceLineItem> _lines = [];
  ...
  Future<void> _loadLookups() async {
    ...
    try {
      final supplierRows = await offlineData.getAll('suppliers');
      ...
      if (mounted) {
        setState(() {
          _isLoading = false;
          _suppliers = supplierRows.map((row) => SupplierModel(
            id: row['id'] as int,
  ...
  Future<void> _showProductSelector() async {
    ...
    if (selected != null) {
      setState(() {
        _lines.add(_PurchaseInvoiceLineItem(
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('تم إنشاء فاتورة المشتريات بنجاح')),
        );
        ...
        Expanded(
          child: ListView.builder(
            ...
            itemBuilder: (ctx, i) {
              ...
              return ListTile(
                title: Text(p.nameAr),
                subtitle: Text('${p.code} - ${p.wholesalePrice.toStringAsFixed(2)} ج.م'),
