...
class CashPaymentCreateScreen extends StatefulWidget {
  const CashPaymentCreateScreen({super.key});

  @override
  State<CashPaymentCreateScreen> createState() => _CashPaymentCreateScreenState();
}

class _CashPaymentCreateScreenState extends State<CashPaymentCreateScreen> {
  ...
  Future<void> _save() async {
    ...
    try {
      await offlineData.create(entityType: 'CashPayment', table: 'cash_payments', data: data);
      setState(() => _isSaving = false);

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('تم إنشاء سند الصرف بنجاح')),
        );
