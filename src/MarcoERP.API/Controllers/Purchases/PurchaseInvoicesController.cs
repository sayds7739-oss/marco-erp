...
public class PurchaseInvoicesController : ApiControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreatePurchaseInvoiceDto dto, CancellationToken ct)
        => FromResult(await _service.CreateAsync(dto, ct));

    [HttpPut]
    public async Task<IActionResult> Update([FromBody] UpdatePurchaseInvoiceDto dto, CancellationToken ct)
