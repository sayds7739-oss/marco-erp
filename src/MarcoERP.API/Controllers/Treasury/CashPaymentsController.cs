...
public class CashPaymentsController : ApiControllerBase
{
    private readonly ICashPaymentService _service;

    public CashPaymentsController(ICashPaymentService service)
    {
        _service = service;
    }
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateCashPaymentDto dto, CancellationToken ct)
        => FromResult(await _service.CreateAsync(dto, ct));

    [HttpPut]
    public async Task<IActionResult> Update([FromBody] UpdateCashPaymentDto dto, CancellationToken ct)
