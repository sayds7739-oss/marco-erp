...
public class CashReceiptsController : ApiControllerBase
{
    private readonly ICashReceiptService _service;

    public CashReceiptsController(ICashReceiptService service)
    {
        _service = service;
    }
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateCashReceiptDto dto, CancellationToken ct)
        => FromResult(await _service.CreateAsync(dto, ct));

    [HttpPut]
    public async Task<IActionResult> Update([FromBody] UpdateCashReceiptDto dto, CancellationToken ct)
